...
class Options:
    def __init__(self, isTrain):
        ...
        self.model['out_c'] = 4
        self.model['suffix']='1020/'# o                  output channel
        self.model['n_layers'] = 4#6   # number of layers in a block
        ...
        # --- training params --- #
        self.train = dict()
        self.train['data_dir'] = '../new_data/data_trainvaltest'  # path to data
        self.train['save_dir'] = './experiments/{:s}/{:s}'.format(self.dataset,self.model['name']+self.model['suffix'])
        self.train['input_size'] = 512   # input size of the image
        self.train['num_epochs'] = 50
        self.train['batch_size'] = 4  if  ('Unet' in self.model['name']) else 2 # batch size
        self.train['val_overlap'] = 80   # overlap size of patches for validation
        self.train['lr'] = 0.0005 if self.dataset == 'her2' else 0.001       # initial learning rate
        self.train['decay_epoch'] = 50#self.train['num_epochs']//2#learning rate decay epoch
        ...
        self.train['gpu'] = [0,1] #if  self.model['name']=='Unet' else [0]     # select gpu devices
        self.train['alpha'] = 1.0        # weight for variance term
        self.train['checkpoint_freq'] = self.train['num_epochs']   # epoch to save checkpoints
        # --- resume training --- #
        ...
        # --- test parameters --- #
        self.test = dict()
        self.test['epoch'] = str(self.train['num_epochs'])
        self.test['gpu'] = [0, ]
        self.test['img_dir'] = 'val/iamges'
        self.test['label_dir'] = 'val/labels'.format(self.dataset)
        self.test['tta'] = True
        self.test['save_flag'] = True
        self.test['patch_size'] = 0 if self.dataset == 'GlaS' else 208
        self.test['overlap'] = 80
        self.test['save_dir'] = './experiments/{:s}/{:s}/{:s}'.format(self.dataset, self.model['name']+self.model['suffix'],self.test['epoch'])
        self.test['model_path'] = './experiments/{:s}/{:s}/checkpoints/checkpoint_{:s}.pth.tar'.format(self.dataset,self.model['name']+self.model['suffix'], self.test['epoch'])
    ...
    def parse(self):
        ...
        if self.isTrain:
            ...
            self.train['img_dir'] = '{:s}/h5'.format(self.train['data_dir'])
